...
class IssueHandler {
  ...
  issueHasIssues () {
    return this.getIssueListIssues() !== ''
  }

  /** Utility method for status reports to issues */
  appendStatusMessage (msg) {
    // TODO: Make the getters use Immutable.JS
    let tmpList = this.getIssueListIssues()
    tmpList += '\`\`\`\n - ' + msg + '\n\`\`\`\n'
    this.setIssueListIssues(tmpList)
  }

  setIssueListIssues (newList) {
    // TODO: Make the getters use Immutable.JS
    let tmpList = this.issue_list.issues
    this.issue_list.issues = newList
  }
